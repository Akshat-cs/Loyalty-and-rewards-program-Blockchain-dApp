...
export default function BuyTickets({ submissions }) {
  ...
  const handleBuyTicket = async (event) => {
    ...
    const pointsKeyType = algosdk.ABIType.from('(address)');
    const pointsKeyPrefix = new Uint8Array(Buffer.from('p-'));
    const pointsKey = new Uint8Array([
      ...pointsKeyPrefix,
      ...pointsKeyType.encode([sender.addr]),
    ]);
    ...
    await NFTticketingdApp.call({
      ...
      { appIndex: 0, name: pointsKey }],
      sender,
    });

    console.log("")
    // Calculate new points
    const newPoints = await NFTticketingdApp.getBoxValue(Buffer.concat([Buffer.from("p-"), algosdk.decodeAddress(sender.addr).publicKey]));

    let result = 0;
    for (let i = 0; i < newPoints.length; i++) {
      result += newPoints[i] * Math.pow(256, newPoints.length - 1 - i);
    }
    console.log("result: ", result);

    // Update points in the context
    updatePoints(result);
  ...
  const handleBuyforpoints = async (event) => {
    const sender = {
      addr: connectedAddress,
      signer: signTxns,
    };
    ...
    const args = [event.asset_ID, { txn: optin, signer: sender.signer }];
          ...
          <div className="col-md-4" key={index} style={{ margin: '30px 0' }}>
            ...
            >
              ...
              <div className="card-body">
                ...
                <p className="card-text">
                  Price of a Ticket in points : {event.ticketPrice} points
                </p>
                <p className="card-text">
                ...
                <button
                  className="btn btn-primary"
                  onClick={() => handleBuyforpoints(event)}
                  style={{ width: '100%', marginTop: '20px' }}
                >
                  Buy for points
                </button>
